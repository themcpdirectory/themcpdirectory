...
export type IdentifierMatchType = "slug" | "alias" | "canonical_registry_name" | "package_identifier";
...
export async function refreshServerSearchDocument(
	...
	const scopedWhere =
		input?.serverId !== undefined ? sql`where s.id = ${input.serverId}` : sql``;
...
export async function getHomepageServers(
	...
	const rows = await db
		...
		.orderBy(sql`${recommendationScoreSql()} desc`, sql`${servers.slug} asc`, sql`${servers.id} asc`)
		.limit(limit)
		.offset(offset);
...
async function resolveAliasLookup(db: QueryDatabase, identifier: string): Promise<ServerIdentifierMatch | null> {
	const rows = await db
		.select({
