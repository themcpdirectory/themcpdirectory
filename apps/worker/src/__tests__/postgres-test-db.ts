...
async function chooseAdminConnectionString(): Promise<string> {
  ...
  throw new Error("Unable to establish a local PostgreSQL admin connection for worker tests.");
}
...
export async function createTempDatabase(prefix = "task5_worker_sync"): Promise<TempDatabase> {
